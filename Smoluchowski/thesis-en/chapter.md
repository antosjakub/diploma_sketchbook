...
where $\beta = 1/k_b T$, and $k_B$ is the Bolztman constant ($\approx 1.2 J$).
...
$$
\partial_t \rho(x,t) = \beta^{-1}\Delta\rho + \nabla\cdot\bigl(\rho\nabla V(x)\bigr) \\
\rho(x,0)=\rho_0(x)
$$ 

This is a linear drift-diffusion equation for the density of the reversible diffusion.
...
---
#### 1. The Linear Potential (Uniform Drift)
$$V(x) = \sum_{i=1}^d c_i x_i$$
*Purpose:* Represents a constant force field (e.g., a uniform electric field pulling ions). The exact solution in an infinite domain is a Gaussian profile that spreads diffusively while its center drifts with velocity $v_d = -D\beta c_i$. This is an excellent transient benchmark to test if a solver introduces artificial numerical dispersion when tracking a moving, unconfined pulse.
- drifting and spreading gaussian profile 
