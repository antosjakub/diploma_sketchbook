...
%tools for describing complex natural phenomena and engineering systems
%Traditional numerical methods, such as the finite element method (FEM)
...
An important subclass is formed by diffusion-type equations, describing the evolution of quantities
...
the sparse grid combination technique.\cite{smolyak,combitech}
Sparse grids combination technique circumvents the curse of dimensionality
by constructing a combination of lower-dimensional anisotropic grids,
thus avoiding the full tensor-product construction of the standard approach.\cite{sg_123}
Under suitable smoothness assumptions on the solution, they provide
an optimal compromise between approximation accuracy and computational cost,
...
and the formulation of the combination technique
...
From the other direction, scientific machine learning has recently
...
Because of this universality, neural networks
have been applied to problems ranging from climate modelling and biological systems to fusion simulations.
...
by Raissi, Perdikaris, and Karniadakis \cite{pinns_original}
...
significantly depends on the choice of network architecture,
and hyperparameters.
Although many new architectural variants and extensions have been proposed,
...
settings still remains an active area of research.\cite{comprehensive_analysis}
...
%% This thesis goal:
This thesis investigates the use of physics-informed neural networks for solving
diffusion-type partial differential equations in moderately high-dimensional settings.
These problems are especially relevant
because a large subclass of problems often live in high-dimensional state spaces.
...
sparse grid combination technique and a few details regarding its implementation.
We then follow by introducing the feed-forward neural networks, before proceeding
to the slightly more involved physics-informed neural networks.

The numerical part of the thesis considers the
diffusion equation formulated in general dimension.
The results presented here are used to evaluate the
accuracy, computational cost, scalability,
and limitations of the considered methods
in a higher-dimensional setting.

%The numerical part of the thesis considers selected diffusion-type problems
%including the force free diffusion, the Smoluchowski equation, and Ornstein-Uhlenbeck processes.
