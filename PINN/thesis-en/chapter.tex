...
One frequently used approach is to adaptively sample the domain proportianally to the value of the residual.
The strategy is a follows:
\begin{enumerate}
\item before the training, sampled a fixed set of points $X$
\item every $K$ epochs:
\begin{enumerate}
\item sample a large pools of candidates points $X_{cand}$
\item evaluate the residual at $X_{cand}$
\item pick $n$ points from $X_{cand}$ with probability proportial to the residual
\end{enumerate}
...
SDGD approximates the full gradient of the physics loss by decomposing the PDE operator into dimension-wise terms
and sampling subsets during training, thus dramatically decreasing the memory requirements of AD and unlocking it applicability even in very high dimensions ($\sim 1000$).
...
$$
\mathcal{L} = \sum_{i=1}^d\frac{\partial^2}{\partial x_i^2}
$$
$$
...
$$
\nabla_\theta \ell_{pde}(\theta) = \nabla_\theta \left( \sum_{i=1}^n \mathcal{L}_i u_\theta(x) - R(x) \right)^2
= 2 \left( \sum_{i=1}^n \mathcal{L}_i u_\theta - R \right) \left( \sum_{i=1}^n \nabla_\theta \mathcal{L}_i u_\theta \right)
$$

The SDGD framework proceeds now as follows:
\begin{enumerate}
\item sample index set $I \subset \{1, \dots, n\}$ with $|I| \ll n$
\item compute detached $\mathcal{L}_i u_\theta$ for all $i$ (forward pass)
\item backpropagate only for $i \in I$ (memory $O(|I|)$ vs. $O(n)$)
\end{enumerate}

The unbiased estimator is then
$$
\nabla_\theta \ell_{pde}^I(\theta) = 2\,\frac{n}{|I|} \left( \sum_{i=1}^n \mathcal{L}_i u_\theta - R \right) \left( \sum_{i \in I} \nabla_\theta \mathcal{L}_i u_\theta \right),
