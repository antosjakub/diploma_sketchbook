...
also reffered to as the expressive power, of the network.
...
Pior to the introduction of PINNs, neural networks were mainly used in the scientific machine learing community
...
construction and tools underlying PINNs is importance before applying them
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{pinn_arch.pdf}
    \caption{}
    \label{fig:pinn_arch}
\end{figure}
...
The training procedure is almost equavalent to to that of feed-forward neural networks.
However, here, we do not have a labeled dataset to iterate over.
In PINNs, we usually proceed in a way that blends together both the minibatch and the fullbatch approaches.
Instead of passing though the entire dataset in every epoch, we instead pass though a relatively small set of
callocation points. Those can be either sampled once, before the training, or repeatedly sampled at each epoch.
Hence the terms collocation points and batch can be used interchangebly.
As a result, instead of \emph{epochs}, we talk of \emph{steps}.
...
\begin{algorithmic}[1]
    ...
    \State \textbf{Initialize} network parameters $\theta$
    \For{step $= 1, \dots, N$}
        \State Randomly shuffle or resample the callocation points
...
as the forward-mode AD scales proportianally to $n$, 
whereas
the reverse-mode AD scales proportianally to $m$.
...
Tanh problem with vanishing gradient, 
tanh standartly used

Silu better for deeper nets.
silu a smooth version fo the frequently used relu activation
$$
...
Following Lulu etc. \cite{deepxde}\cite{rar}, the method proceeds as follows
...
PyTorch supports multiple optins for computing the gradinet w.r.t. the model inputs.
One such opitons is forward over revere and reverse over forward and hessian and then reduce.
- functinal, efficient.
The network output and its required derivatives are computed with

All construct the hessian in some way which reguires $O(d^2)$ storage.

Also suppose \texttt{torch.autograd.grad}
can be computed in a for loop

Snipper of the gradient and laplce calculation using AD in torch.
...
As for code optimizaiton, torch profiler in conjuction with the
perfetto visualization software were utilized.

In particular, perfetto allowed the discovery of a serious 
performace issue of the native torches Dataloader class.

As a result, the pipepile was altered to preparet the required
data manually shuffle and prepare instead of relying on the default
dataloader batch iteration which is optimized for handling 
much larger and more complex deep learning datasets.
...
\begin{figure}
    ...
    \caption{Screenshot from Perfetto, which enables visual inspection of elementary Torch operations.
    Note the \texttt{grad\_u} and \texttt{laplace\_u} labels at the bottom, corresponding to the \texttt{record} marking in the provided code snippet.}
    \label{fig:perfetto}
\end{figure}
