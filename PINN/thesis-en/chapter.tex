...
Parametrize the solution by a nn $\mathcal N_\theta=u_\theta$
define the NN as so:
$d_{in} = d+1 =: D$
$d_{out} = 1$
...
What remains is to define the strategy for computing the loss.
This is what separates physics neural networks from other approaches of modelling physical systems.

Returning to the system above, we defing a pde, bc, and ic residuals at a space time coordinate $(x,t)$ as
\begin{align*}
...
The loss the neural network then seeks to minimize is a linear combiantion of the individual losses:
\begin{align*}
\ell = \lambda_{pde}\ell_{pde} + \lambda_{bc}\ell_{bc} + \lambda_{ic}\ell_{ic}
\end{align*}

where $\lambda_r>0$, and the individual losses are computed as a mean squared error over the residuum
\begin{align*}
\ell_{pde}\left(\theta;\{(x_i,t_i)\}_{i=1}^{N_{pde}}\right)
= \frac{1}{N_{pde}}\sum_{i=1}^{N_{pde}} \big(\mathcal R_{pde}(u_\theta;x_i,t_i)\big)^2,\quad
\{(x_i,t_i)\}_{i=1}^{N_{pde}} \subset (0,T)\times\Omega\\
\ell_{bc}\left(\theta;\{(x_i,t_i)\}_{i=1}^{N_{bc}}\right)
= \frac{1}{N_{bc}}\sum_{i=1}^{N_{bc}} \big(\mathcal R_{bc}(u_\theta;x_i,t_i)\big)^2,\quad
\{(x_i,t_i)\}_{i=1}^{N_{bc}} \subset (0,T)\times\partial\Omega\\
\ell_{ic}\left(\theta;\{x_i\}_{i=1}^{N_{ic}}\right)
= \frac{1}{N_{ic}}\sum_{i=1}^{N_{ic}} \big(\mathcal R_{ic}(u_\theta;x_i)\big)^2,\quad
...
In the context of PINN, we don't talk of training data since there is no explicit target value $y_i$ for the input $x_i$.
Instead, we seek to minimize the residuum over a chosen set of points spanning the domain.
We call such a set of points a \emph{callocation} set, or just \emph{callocaiton} points.
Our dataset thus consist of just a set of points from the domain.
...
\begin{algorithmic}[1]
    ...
    \item callocation points
    \begin{itemize} 
    \item $X_{pde}=\{(x_i, t_i)\}_{i=1}^{N_{pde}}$
    \item $X_{bc}=\{(x_i, t_i)\}_{i=1}^{N_{bc}}$
    \item $X_{ic}=\{(x_i)\}_{i=1}^{N_{ic}}$
    \end{itemize} 
