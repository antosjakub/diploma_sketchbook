...
\node[align=center, below=1cm of output] (data) {
  ...
  $(0,T)\times\Omega,\: \Omega,$\\[0.0cm]
  $(0,T)\times\partial\Omega$%\\[0.0cm]
  %$\Omega$
};
...
\node[right=0.6cm of ad] (r_pde) {$\ell_{pde} = ||\partial_t u_\theta + \mathcal{N}(u_\theta)||^2_{(0,T)\times\Omega}$};
\node[above right=0.05cm and 0.6cm of ad] (r_ic) {$\ell_{ic} = ||u_\theta - u_{ic}||^2_{\Omega}$};
\node[above right=0.5cm and 0.6cm of ad] (r_bc) {$\ell_{bc} = ||u_\theta - u_{bc}||^2_{(0,T)\times\partial\Omega}$};
