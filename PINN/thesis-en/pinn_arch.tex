...
\node[input, below=0.1cm of input1] (input2) {...};
\node[input, below=0.1cm of input2] (input3) {$x_d$};
\node[input, below=0.1cm of input3] (input4) {$t$};
...
% Output
\node[output, right=0.8cm of g3] (output) {$u_\theta(x,t)$};
...
\node[align=center, below=1cm of output] (data) {
  ...
  $X_{pde}\sim\Omega\times(0,T)$\\[0.0cm]
  $X_{bc}\sim\partial\Omega\times(0,T)$\\[0.0cm]
  $X_{ic}\sim\Omega$
  %$\Omega$
};
...
\node[right=0.6cm of ad] (r_pde) {$\ell_{pde} = \left\langle (\partial_t u_\theta + \mathcal{L}(u_\theta))^2\right\rangle_{X_{pde}}$};
\node[above right=0.05cm and 0.6cm of ad] (r_ic) {$\ell_{ic} = \left\langle(u_\theta - u_{ic})^2\right\rangle_{X_{ic}}$};
\node[above right=0.5cm and 0.6cm of ad] (r_bc) {$\ell_{bc} =  \left\langle(u_\theta - u_{bc})^2\right\rangle_{X_{bc}}$};
...
$\partial_t u + \mathcal{L}(u) = 0$\\[0.0cm]
$u = u_{ic}$\\[0.0cm]
$u = u_{bc}$
};
