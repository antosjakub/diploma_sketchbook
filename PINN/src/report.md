Here is the adapted technical report. I have updated the computational paradigms, automatic differentiation strategies, and pseudocode to specifically target **PyTorch** instead of JAX. Per my system formatting guidelines, I have also converted the structural LaTeX into clean Markdown while preserving all of your mathematical notation, structure, and citation markers.

***

# Physics-Informed Neural Networks for High-Dimensional Diffusion-Type PDEs:
### A Technical Report Oriented Toward PyTorch Implementation

## Conceptual Summary

Physics-informed neural networks (PINNs) are neural-network ansätze for unknown solution fields $u(t,x)$ that are trained not mainly from labeled data, but from the requirement that the network output satisfy a governing PDE together with its initial and boundary conditions.[file:5][file:6]  
...
The original PINN paper already emphasized that the same framework can handle nonlinear PDEs, inverse problems, periodic conditions, and complex-valued solutions such as the nonlinear Schrödinger equation.[file:5]

However, the most important practical lesson from the attached papers is that *plain* PINNs are often hard to train.[file:3][file:6]  
...
Therefore, for the class of problems that motivated this report, the right question is not "do PINNs exist for high-dimensional PDEs?", but rather "which PINN *variant* should be used, and with which training pipeline?".[file:6][file:3][file:2]  
...
For your use case—forward numerical solution of diffusion, heat, Fokker–Planck, and Schrödinger-type equations in dimensions roughly $5$ to $30+$, with no observational data—PINNs are best viewed as a family of meshless residual-minimization methods rather than as a single algorithm.[file:5][file:6]  
...
A high-performance PyTorch implementation should therefore be organized around a modular PINN framework in which the PDE, the residual estimator, the boundary handling strategy, the network architecture, and the sampling/training schedule are all swappable components.[file:3][file:2][file:4][file:6]
...
$$\partial_t u + \mathcal{N}[u](t,x) = 0, \qquad (t,x)\in (0,T]\times \Omega,$$
with initial condition
$$u(0,x)=u_0(x), \qquad x\in \Omega,$$
and boundary condition
$$\mathcal{B}[u](t,x)=g(t,x), \qquad (t,x)\in [0,T]\times \partial\Omega.$$
...
and Schrödinger-type equations, where one often deals with complex-valued outputs or coupled real–imaginary systems.[file:6][file:2][file:5]
...
For scalar diffusion equations one typically uses $u_\theta:[0,T]\times \Omega \to \mathbb{R}$, while for Schrödinger equations one often outputs $(u_\theta^{(R)},u_\theta^{(I)})$ as a two-output real network.[file:5]
...
### Residual construction by automatic differentiation
...
If
$$\mathcal{R}_\theta(t,x) := \partial_t u_\theta(t,x) + \mathcal{N}[u_\theta](t,x),$$
...
In PyTorch terms, this means that the neural network is typically implemented as an `nn.Module`, and all PDE operators are computed using `torch.autograd.grad` (with `create_graph=True` to allow higher-order derivatives).[file:4][file:3]  
...
where
$$\mathcal{L}_r(\theta) = \frac{1}{N_r}\sum_{n=1}^{N_r} \bigl|\mathcal{R}_\theta(t_n^{(r)},x_n^{(r)})\bigr|^2,$$
$$\mathcal{L}_{ic}(\theta) = \frac{1}{N_{ic}}\sum_{n=1}^{N_{ic}} \bigl|u_\theta(0,x_n^{(ic)})-u_0(x_n^{(ic)})\bigr|^2,$$
and
$$\mathcal{L}_{bc}(\theta) = \frac{1}{N_{bc}}\sum_{n=1}^{N_{bc}} \bigl|\mathcal{B}[u_\theta](t_n^{(bc)},x_n^{(bc)})-g(t_n^{(bc)},x_n^{(bc)})\bigr|^2.$$
...
Later practice, especially in modern PyTorch pipelines and higher-dimensional settings, uses Adam or another first-order optimizer with random mini-batches, often followed by L-BFGS (via `torch.optim.LBFGS`) as a refinement stage when feasible.[file:3][file:6]
...
First, standard coordinate MLPs have a *spectral bias*: they learn low-frequency components first and struggle with high-frequency, multiscale, or sharply varying structure.[file:3][file:6]  
...
The *continuous-time* formulation enforces the PDE directly through the residual $\mathcal{R}_\theta(t,x)$ on space-time points.[file:5]  
This is the form used in most modern work on high-dimensional PINNs and is the main focus of this report, because it is simple, general, and easy to express natively in PyTorch.[file:2][file:3][file:4]

The *discrete-time* formulation uses Runge–Kutta structure and predicts one or more time stages, effectively embedding a time integrator into the network architecture.[file:5]  
...
## Architectures and Framework Variants Relevant in High Dimension
...
### Adaptive activation architectures
...
The idea is to replace a fixed activation shape by a trainable one, so that the local nonlinearity of the network can adjust during training (e.g., using `nn.Parameter` inside custom activation modules).[file:6]  
...
If implemented in PyTorch, they are cheap compared with more radical changes such as second-order operator estimators, so they are worth keeping as an optional switch.[file:6]

### SIREN and oscillatory activations
...
This matters for Schrödinger-type equations, wave-like diffusive-dispersive systems, and highly oscillatory multi-scale regimes where $\tanh$ may smooth too aggressively.[file:6]  
...
The trade-off is that oscillatory activations can be harder to initialize properly (requiring specific initialization schemes) and may interact subtly with higher-order derivatives.[file:6]  
...
For Schrödinger or mixed diffusive-oscillatory problems, they become much more interesting.[file:6][file:5]
...
with a frequency matrix $B$ sampled from a Gaussian distribution and held constant.[file:3]  
...
For diffusion-type PDEs in $d=5$ to $30$, Fourier features are one of the most practical upgrades because they are easy to implement (as an initial unlearned `nn.Linear` layer), PyTorch-friendly, and supported by both theory and extensive experiments.[file:3]  
...
The guide explains that this changes the geometry of the optimization landscape and effectively induces self-adaptive per-neuron learning rates (achievable in PyTorch using `torch.nn.utils.weight_norm` or custom linear modules).[file:3]

Empirically, RWF often accelerates convergence and improves final accuracy.[file:3]  
Because it is a drop-in modification of dense layers, it is very attractive in PyTorch: it adds little conceptual complexity and can be composed naturally with Fourier features, modified MLPs, and causal training.[file:3]  
...
In a PyTorch implementation, this is a good candidate for the default backbone, especially when combined with Fourier features and RWF.[file:3]

### Hard-constrained architectures for conditions

A major architectural issue in PINNs is whether initial and boundary conditions are enforced *softly* through penalty losses or *hardly* by building them into the network output.[file:6][file:3][file:2]  
...
### Variational and weak-form architectures
...
For a PyTorch codebase oriented toward research flexibility, a weak-form backend is highly desirable even if the first implementation uses the strong form.[file:6]

### Domain decomposition: cPINN, XPINN, and related methods
...
A research-grade PyTorch implementation should therefore isolate domain decomposition as a first-class module rather than hard-coding a single global network.[file:6][file:2]

### FBPINN and finite-basis localized subnetworks
...
Compared with one monolithic global MLP, FBPINN offers better scalability and a natural path to parallel training (e.g., utilizing PyTorch's `DistributedDataParallel` natively).[file:6]
...
### Time-aware architectures: causal weighting, PPINN, Trans-Net, and windows
...
### High-dimensional residual architectures: SDGD
...
In a PyTorch implementation, SDGD should be treated as a residual-estimator backend that sits underneath the same model and optimizer interface as the ordinary exact residual.[file:2]

### HTE-based residual architectures
...
This can avoid forming the full Hessian and substantially reduce memory cost, especially when implemented efficiently in PyTorch using `torch.autograd.functional.hvp` or via forward-mode AD (`torch.autograd.forward_ad`).[file:4]
...
The paper compares HTE with SDGD and argues that each has regimes in which it is preferable, with HTE being particularly appealing when the operator naturally appears as a trace and efficient PyTorch implementation is available.[file:4]  
Since you explicitly want a PyTorch-oriented report, HTE deserves a central place in the eventual code design.[file:4]

### Score-PINN for Fokker–Planck equations
...
The paper also connects this strategy to efficient derivative estimation frameworks and shows that it can outperform direct PINN approaches to the log-likelihood formulation.[file:1]  
...
For a first high-performance PyTorch solver, however, Bayesian PINNs should be considered future work rather than core infrastructure.[file:6]

### Hybrid deep-learning architectures: LSTM, CNN, GAN, DeepONet, transfer and meta-learning
...
### Practical ranking for your target problems
...
1. Fourier features + modified MLP + RWF as the default coordinate network.[file:3]
2. Hard-constrained handling of conditions whenever simple geometry permits it.[file:3][file:6]
3. Causal weighting or time-windowing for longer time horizons.[file:3][file:6]
4. SDGD for Laplacian-like operators, and HTE when the operator is naturally trace-based.[file:2][file:4]
5. XPINN/FBPINN/domain decomposition when the global model remains hard to optimize.[file:6]
...
For Schrödinger equations, the baseline should be adjusted toward complex or two-output networks, periodic hard constraints when appropriate, and possibly oscillatory activations or frequency-rich embeddings.[file:5][file:6]  
...
## Most Important Ideas for High-Dimensional PDEs
...
Therefore, the first high-dimensional design principle is: *do not compute the full operator naively if the PDE structure allows stochastic decomposition or trace estimation*.[file:2][file:4]
...
For Fokker–Planck operators with trace structure, HTE can often provide a more PyTorch-native path using Hessian-vector products.[file:4]  
...
For strongly oscillatory Schrödinger-type problems, frequency-rich architectures or oscillatory activations become more relevant than for smoothing heat equations.[file:5][file:6]

Finally, one should be realistic about the role of PINNs.[file:6]  
The review explicitly notes that baseline PINNs often do *not* yet match mature classical solvers in generic accuracy, and that breakthroughs usually come from problem-specific enhancements rather than from a single universal architecture.[file:6]  
...
## Concrete Application: Heat Equation with Diffusion and Source

### Model problem

Consider the $d$-dimensional inhomogeneous heat equation on the box $\Omega=[-1,1]^d$:
$$\partial_t u(t,x) - \kappa \Delta_x u(t,x) = f(t,x), \qquad (t,x)\in (0,T]\times \Omega,$$
with initial condition
$$u(0,x)=u_0(x), \qquad x\in \Omega,$$
and Dirichlet boundary condition
$$u(t,x)=g(t,x), \qquad (t,x)\in [0,T]\times \partial\Omega.$$
...
$$R_\theta(t,x) = \partial_t u_\theta(t,x)-\kappa \sum_{i=1}^d \partial_{x_i x_i}u_\theta(t,x)-f(t,x).$$
...
$$\mathcal{L}(\theta) = \lambda_r \mathcal{L}_r + \lambda_{ic}\mathcal{L}_{ic} + \lambda_{bc}\mathcal{L}_{bc}.$$
...
### Recommended network and training stack

For a first serious PyTorch implementation, the attached papers suggest the following default stack:
1. Non-dimensionalize the PDE so that $t$, $x_i$, and $u$ are all $O(1)$.[file:3]
2. Use Fourier feature embeddings on the coordinates, especially when the solution contains moderate or high spatial frequency content.[file:3]
3. Use a modified MLP backbone with $\tanh$ activations and random weight factorization.[file:3]
4. Use Adam (`torch.optim.Adam`) with learning-rate decay and random re-sampling of collocation points every iteration.[file:3]
5. Use adaptive loss balancing if IC and BC are imposed softly.[file:3][file:6]
6. Use causal weighting or time windows when $T$ is large.[file:3][file:6]
...
Hence the heat equation is one of the cleanest testbeds for SDGD in a PyTorch PINN codebase.[file:2]

### Sampling plan

At each training iteration, sample three point sets if using soft IC/BC constraints:
$$\{x_n^{(ic)}\}_{n=1}^{N_{ic}} \subset \Omega, \qquad \{(t_n^{(bc)},x_n^{(bc)})\}_{n=1}^{N_{bc}} \subset [0,T]\times \partial\Omega,$$
...
### Residual computation in PyTorch

Suppose the raw network is an `nn.Module` yielding $u_\theta(t,x)$ with scalar output.[file:4][file:3]  
The implementation should separate:
1. The model class returning $u_\theta(t,x)$.[file:4]
2. A wrapper that builds the hard-constrained or soft-constrained physical solution.[file:3][file:6]
3. A residual function that uses `torch.autograd.grad` to compute $\partial_t u_\theta$ and $\Delta_x u_\theta$.[file:2][file:4]

For exact Laplacian mode, one computes
$$\Delta_x u_\theta(t,x)=\sum_{i=1}^d \partial_{x_i x_i}u_\theta(t,x).$$
In PyTorch this is conceptually straightforward using `grad(outputs=du_dx, inputs=x, create_graph=True)`, but for larger $d$ it becomes expensive because every second derivative participates in the computational graph for backpropagation.[file:2][file:4]

For SDGD mode, write
$$\Delta_x u_\theta = \sum_{i=1}^d L_i u_\theta, \qquad L_i u_\theta = \partial_{x_i x_i}u_\theta,$$
...
$$\mathcal{L}_r = \frac{1}{N_r}\sum_{n=1}^{N_r} |R_\theta(t_n^{(r)},x_n^{(r)})|^2,$$
$$\mathcal{L}_{ic} = \frac{1}{N_{ic}}\sum_{n=1}^{N_{ic}} |u_\theta(0,x_n^{(ic)})-u_0(x_n^{(ic)})|^2,$$
$$\mathcal{L}_{bc} = \frac{1}{N_{bc}}\sum_{n=1}^{N_{bc}} |u_\theta(t_n^{(bc)},x_n^{(bc)})-g(t_n^{(bc)},x_n^{(bc)})|^2.$$
...
1. Initialize the network with Glorot initialization, Fourier features enabled, and RWF-enabled dense layers.[file:3]
2. For each iteration, resample interior, boundary, and initial points.[file:3][file:6]
3. Evaluate residual, IC, and BC losses using PyTorch's automatic differentiation.[file:5][file:3]
4. Update adaptive loss weights if using them.[file:3][file:6]
5. Zero gradients, call `loss.backward()`, and apply Adam step with decay.[file:3]
6. Every fixed number of steps, compute diagnostics: relative sizes of losses, gradient norms, and, if possible, residual histograms across time or subdomains.[file:3]
7. If training stagnates, either increase residual points, activate adaptive sampling, or switch to time-windowed training.[file:6][file:3]

For lower dimensions and moderate batch sizes, one may optionally finish with L-BFGS (`torch.optim.LBFGS`) after Adam plateaus.[file:5][file:6]  
...
### Pseudocode for a PyTorch-oriented implementation
...
The important design choice is that `mode_residual` must be modular.[file:2][file:4]  
...
1. Heat equation on $[-1,1]^d$ with zero Dirichlet BC, exact Laplacian, and soft IC/BC losses.[file:5][file:3]
2. Add hard BC via $\phi(x)=\prod_{i=1}^d (1-x_i^2)$.[file:3][file:6]
3. Add Fourier features, modified MLP, and RWF.[file:3]
4. Add adaptive loss balancing and causal weighting.[file:3]
5. Add SDGD for the Laplacian.[file:2]
6. Add time-windowing for larger $T$.[file:3][file:6]
7. Generalize from $\kappa \Delta u$ to anisotropic diffusion $\nabla\cdot(A(x)\nabla u)$.[file:6]
8. Add Fokker–Planck support, then consider HTE and Score-PINN as specialized backends.[file:4][file:1]
...
For high-dimensional diffusion-like PDEs, PINNs should be implemented as a *framework of choices* rather than as a single monolithic solver.[file:6][file:3][file:2]  
...
If the eventual PyTorch code is built around those stateful `nn.Module` abstractions and flexible autograd graphs from the start, it will be well positioned not only for heat and classical diffusion equations, but also for Fokker–Planck, Schrödinger, and later operator-learning extensions.[file:1][file:5][file:6]

***

Would you like me to draft out the specific PyTorch code for the `SDGD` residual estimator, or focus on detailing the `Modified MLP` architecture?